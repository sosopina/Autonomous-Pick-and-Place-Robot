  ...
  if (deviceOk)
  {
    ...
    for (int col=0;col<width;col++) {
      ...
      if (ok) {
        ...
        powCol+=col*col;
        sumDepth+= depth;
        powDepth+=depth*depth;         
      }
    } // for
    bool ok=(nbCol>NB_COL_MIN)&&(nbCol<NB_COL_MAX);
    if (ok) {
      powDepth/=nbCol;meanDepth=sumDepth/ nbCol;
      powCol/=nbCol;meanCol=sumCol/nbCol;
      varCol= powCol-meanCol *meanCol;
      ok=varCol< MAX_SD_COL*MAX_SD_COL;
    }  
...
void KobukiManager::updateMeasureRGB(uint8_t *frame_data,unsigned long long frame_number)
{
  ...
  for (int col = width/2, k = k0+3*col; col < width; col++)
  {
    ...
    // stop search if one line has ever been found  
      if (nb_ok>0) break;
    }
  }
  for (int col = width/2-1, k = k0+3*col; col >= 0; col--)
  {
    uint8_t r = frame_data[k];
    uint8_t g = frame_data[k+1];
    uint8_t b = frame_data[k+2];
    k-=3;
    bool ok = (r > seuil_rgb) && (g > seuil_rgb) && (b > seuil_rgb);
    if (ok)
    {
  ...
  control.measure = -(scaleMeter * (mean_col2 - width / 2)) / scalePixel;
